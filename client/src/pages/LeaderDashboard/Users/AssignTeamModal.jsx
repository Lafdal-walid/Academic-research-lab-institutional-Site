...
const AssignTeamModal = ({ isOpen, onClose, user, onUpdate }) => {
    ...
    const fetchTeams = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setTeams(data);
                if (user && user.team && user.team !== 'Not Assigned') {
                    const currentTeam = data.find(t => t.name === user.team);
                    if (currentTeam) setSelectedTeamId(currentTeam._id);
                }
            }
        } catch (err) {
    ...
    const handleAssign = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/auth/admin/users/${user.id}`, {
                ...
                body: JSON.stringify({ team: selectedTeamId })
            });
            ...
            <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
                ...
                >
                    ...
                    <div className="flex items-center gap-[12px] w-full">
                        ...
                        <h2 className="font-['Gilroy',sans-serif] font-bold text-white text-[16px]">Assign Research Team</h2>
                    ...
                    <p className="font-['Poppins',sans-serif] text-[14px] text-[#a5a5b2] w-full leading-[1.6]">
                        Modify team membership for <strong>{user?.username}</strong>.
                    </p>
                    ...
                    <div className="flex flex-col gap-[20px] items-start w-full">
                        ...
                        <div className="flex flex-col gap-[12px] w-full relative">
                            <label className="font-['Poppins',sans-serif] text-[#80808a] text-[14px]">Select Team</label>
                            <div 
                                className={`bg-[rgba(255,255,255,0.01)] h-[41px] relative rounded-[8px] w-full border ${isDropdownOpen ? 'border-[#3457DC]' : 'border-[#2a2a30]'} px-[14px] flex items-center justify-between cursor-pointer transition-colors`}
                                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                            >
                                <span className="text-white text-[14px] font-['Poppins',sans-serif]">{selectedTeamName}</span>
                                <div className={`text-[#3457DC] transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`}>
                        ...
                        <div className="flex gap-[8px] items-start w-full opacity-70">
                            ...
                            <p className="font-['Poppins',sans-serif] text-[#a5a5b2] text-[12px] flex-1">
                                Assigning a team will categorize the user's research output under that specific unit.
                            </p>
                        </div>
